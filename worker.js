...
userQueue.process(async (job, done) => {
  ...
  try {
    const email_subkect = 'Welcome to ALX-Files_Manager by Kwenziwa';
    const email_body = [
      '<div>',
      '<h3>Hello {{user.name}},</h3>',
      'Welcome to <a href="https://github.com/kwenziwa/alx-files_manager">',
      'ALX-Files_Manager</a>, ',
      'a simple file management API built with Node.js by ',
      '<a href="https://github.com/Kwenziewa">Kwenziwa Khanyile</a>. ',
      'We hope it meets your needs.',
      '</div>',
    ].join('');
    Mailer.sendMail(Mailer.buildMessage(user.email, email_subkect, email_body));
    done();
  } catch (err) {
    done(err);
  }
